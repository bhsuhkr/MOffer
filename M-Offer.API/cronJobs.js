...
function runDailySummary() {
  ...
  sql
    ...
    .then(async (pool) => {
      ...
      try {
        pool.query(`exec sp_processDailySummary '` + summaryDate + `', 1`, (err, recordset) => {
            ...
            if (err) console.log(err);
              ...
              if (recordset && recordset.recordsets && recordset.recordsets[0] && recordset.recordsets[0][0]) {
                ...
                for (index = 0; index < recordset.recordsets[0].length; index++) {
                  console.log(
                    "daily summary record ID: ",
                    recordset.recordsets[0][index].DSID,
                    " for date --",
                    summaryDate
                  );
                  if (index === 0) {
                    htmlContent =
                      "<p>This email is for <b>New Song POS Reconciliation</b>." +
                      "<p><b>Here is Today's (" +
                      summaryDate +
                      ") Summary:</b><br>" +
                  ...
                  htmlContent =
                    ...
                    recordset.recordsets[0][index].TransPoint +
                    "</b></font>" +
                    "<td align=right><font color='blue'><b>" +
                    USDollar.format(recordset.recordsets[0][index].DailyTotalCreditAmount) +
                    "</b></font>" +
                    "<td align=right><font color='blue'><b>" +
                    USDollar.format(recordset.recordsets[0][index].DailyTotalDebitAmount) +
                    "</b></font>" +
                    "<td align=right><font color='blue'><b>" +
                    USDollar.format(recordset.recordsets[0][index].DailyTotalRefundAmount) +
                    "</b></font>" +
                    "<td align=right><font color='blue'><b>" +
                    USDollar.format(recordset.recordsets[0][index].DailyBalance) +
                    "</b></font>" +
                    "<td align=center><font color='blue'><b>" +
                    recordset.recordsets[0][index].DailyActiveMembers +
                    "</b></font>" +
                    "<td align=center><font color='blue'><b>" +
                    recordset.recordsets[0][index].DailyNewMembers +
                    "</b></font>" +
                    "<td align=center><font color='green'><b>" +
                    recordset.recordsets[0][index].GrandTotalActiveMembers +
                    "</b></font>" +
                    "<td align=right><font color='green'><b>" +
                    USDollar.format(recordset.recordsets[0][index].GrandTotalMemberBalance) +
                    "</b></font>" +
                    "<td align=right><font color='green'><b>" +
                    USDollar.format(recordset.recordsets[0][index].GrandTotalTransBalance) +
                    "</b></font>" +
                    "</tr>";
                ...
                if (htmlContent.length > 0) {
                  ...
                  transporter
                    ...
                    .then(
                      console.log(
                        "Right before sending, email host verified",
                        config.emailHost,
                        "connected using",
                  ...
                  transporter
                    ...
                    .then((info) => {
                      //console.log({ info });
                      console.log(
                        "Cron job runDailyEmailRecon sent email to",
                        config.cronDailyEmailReconToAddress,
                        "at",
                ...
                console.log(
                  "Cron job runDailyEmailRecon did not have any records to send at",
                  new Date().toLocaleString()
                );
              }
            }
...
function runDailyEmailMemberReceipt() {
  ...
  sql
    ...
    .then(async (pool) => {
      ...
      try {
        pool.query(
          `select distinct memberId from nc_transactions where CONVERT(DATE, TransTime) = '2024-01-14'`,
          (err, recordset) => {
            if (err) console.log(err);
            else {
              if (recordset && recordset.recordsets && recordset.recordsets[0] && recordset.recordsets[0][0]) {
